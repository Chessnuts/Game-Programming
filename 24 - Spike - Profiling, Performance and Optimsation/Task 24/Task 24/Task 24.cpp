...
{	//yeap - lazyfoo style!
	...
	*/
	...
	int leftA = boxes[i].x;
	int rightA = boxes[i].x + boxes[i].w;
	int topA = boxes[i].y;
	int bottomA = boxes[i].y + boxes[i].h;

	//Calculate the sides of rect B
	int leftB = boxes[j].x;
	int rightB = boxes[j].x + boxes[j].w;
	int topB = boxes[j].y;
	int bottomB = boxes[j].y + boxes[j].h;
...
int main(int argc, char* args[])
{
	...
	{
		/*
		(void)run_test("Test A1", crash_test_all_A1); //TODO: not paying attention to return values. :(
		(void)run_test("Test A2", crash_test_all_A2);
		(void)run_test("Test B", crash_test_all_B);
		(void)run_test("Test C", crash_test_all_C);
		*/
		(void)run_test("Test D", crash_test_all_D);
	}
