...
void PlayAdventure::LoadAdventure(string fileName)
{
    ...
    while (getline(fs, str))
    {
        ...
        if ((str.size() != 0) && (str.at(0) == 'E'))
        {
            ...
            {
                vector<string> details = split(str, '|');
                locations.back().entities.push_back(Entity{ split(details[1], ','), details[2], details[3],  split(details[4], ',') });
                if ((details.size() > 6) && (details.at(5) == "I"))
                {
                    ...
                    for (auto i : items)
                    {
                        ...
                        locations.back().entities.back().inventory.Put(new Entity{ split(d[1], ','), d[2], d[3],  split(d[4], ',') });
                    }
                }
            }
        }
